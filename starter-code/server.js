...
app.get('/articles/all', function(request, response) {
  ...
  client.connect(function(err) {
    ...
    client.query(
      ...
      FROM authors
      INNER JOIN articles
      ON authors.author_id=article.article_id;
...
app.post('/articles/insert', function(request, response) {
  ...
  function queryTwo() {
    client.query(
      `SELECT author, authorUrl FROM authors(needswork)
      ;`, // : Write a SQL query to retrieve the author_id from the authors table for the new article
  ...
  function queryThree(author_id) {
    client.query(
      `INSERT article INTO author_id(needswork);`, // TODO: Write a SQL query to insert the new article using the author_id from our previous query
...
app.put('/articles/update', function(request, response) {
  ...
  client.query(
    `SELECT author_id
    FROM authors
    WHERE VALUES=$1;`, // TODO: Write a SQL query to retrieve the author_id from the authors table for the new article
  ...
  function queryTwo(author_id) {
    client.query(
      `UPDATE author
      SET title = $1 category = $2 author = $3 authorUrl = $4 publishedOn = $5 body = $6;`, // TODO: Write a SQL query to update an existing author record
      [
        request.body.title,
  ...
  function queryThree(author_id) {
    client.query(
      `UPDATE article
      SET title = $1 category = $2 author = $3 authorUrl = $4; publishedOn = $5 body =$6;`, // TODO: Write a SQL query to update an existing article record
      [
        request.body.title,
        request.body.category,
        request.body.author,
